    ...
    <el-card>
      <el-table :data="userList" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="180"> 
          <template slot-scope="scope">
            <el-button @click="openSaveOrEditUI(scope.row.id)" type="primary" icon="el-icon-edit" size="mini" circle></el-button>
  ...
  methods: {
    ...
    saveOrEdit() {
      ...
      this.$refs.userFormRef.validate((valid) => {
        if (valid) {
          // 提交请求给后台
          userApi.saveOrEditUser(this.userForm).then(response => {
            // 成功提示
            this.$message({
              message: response.message,
              type: 'success'
